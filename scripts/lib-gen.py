...
def rev_comp(seq):
    revcomps = {'A': 'T', 'T': 'A', 'G': 'C', 'C': 'G'}
    return ''.join([revcomps[x] for x in seq][::-1])
...
for x_name, y_name in pairs:
    x_dna, x_loops = generate_seq(peptides[x_name], codon_dict, args.f_primer.upper(), spacer[:mer], re_sites, flip=False, revcomp=args.rc_x)
    x_acc.append(x_loops)
    y_dna, y_loops = generate_seq(peptides[y_name], codon_dict, args.r_primer.upper(), spacer[-mer:], re_sites, flip=True, revcomp=args.rc_y)
